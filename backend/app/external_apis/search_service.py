...
from .intelligent_search_router import IntelligentSearchRouter, NewsSearchRouter, SearchProviderChoice
...
class SearchProvider(Enum):
    ...
    SERPAPI = "serpapi"
    BRAVE = "brave"
    AUTO = "auto"  # Intelligent routing based on query analysis
...
class SearchService:
    ...
    def __init__(self):
        ...
        self.search_router = IntelligentSearchRouter()
        self.news_router = NewsSearchRouter()
        
        # Provider priority: Now determined by intelligent routing
        # Brave Search = Primary for general/real-time queries
        # SerpAPI = Primary for structured/specialized queries
    ...
    async def search_web(
        ...
        """
        Perform web search with intelligent provider selection and fallback.
        
        Strategy:
        - AUTO mode: Analyzes query and routes to optimal provider
          * Brave Search: General queries, real-time info, news
          * SerpAPI: Structured data, rich metadata, specialized searches
        - Manual mode: Uses specified provider with fallback
        ...
        Returns:
            List of search results with unified format and routing metadata
        """
        if not query.strip():
        ...
        if provider == SearchProvider.AUTO:
            strategy = self.search_router.get_search_strategy(query)
            logger.info(
                f"Intelligent routing: {strategy['primary_provider'].value} "
                f"(confidence: {strategy['confidence']:.2f}) - {strategy['reasoning']}"
            )
            
            # Try primary provider first
            primary_provider = strategy['primary_provider'].value
            fallback_provider = strategy['fallback_provider'].value
            
            providers_to_try = [
                (SearchProvider(primary_provider), 
                 self.serpapi if primary_provider == 'serpapi' else self.brave),
                (SearchProvider(fallback_provider),
                 self.serpapi if fallback_provider == 'serpapi' else self.brave)
            ]
        else:
    ...
    async def search_news(
        ...
        """
        Perform news search with intelligent provider selection and fallback.
        
        Strategy:
        - AUTO mode: Analyzes news query and routes to optimal provider
          * Brave Search: Breaking news, real-time updates, general news (faster)
          * SerpAPI: Trending news, rich metadata, news feed with popularity signals
        - Manual mode: Uses specified provider with fallback
        ...
        Returns:
            List of news results with unified format and routing metadata
        """
        if not query.strip():
            logger.warning("Empty news search query provided")
            return []
        
        # Intelligent news query analysis for AUTO mode
        if provider == SearchProvider.AUTO:
            strategy = self.news_router.get_news_strategy(query)
            logger.info(
                f"News routing: {strategy['primary_provider'].value} "
                f"(confidence: {strategy['confidence']:.2f}) - {strategy['reasoning']}"
            )
    ...
    def analyze_query(self, query: str, search_type: str = "web") -> Dict[str, Any]:
        """
        Analyze query and return routing decision without executing search.
        ...
        Returns:
            Dict with routing decision, reasoning, and confidence
        """
        if search_type == "news":
            strategy = self.news_router.get_news_strategy(query)
        else:
        ...
        return {
            ...
            "explanation": f"Query will be routed to {strategy['primary_provider'].value} "
                          f"because: {strategy['reasoning']}"
        }
    ...
    async def get_provider_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "recommended": None,
            "routing_info": {
                "strategy": "Intelligent routing based on query analysis",
                "brave_use_cases": [
                    "General web search",
                    "Real-time information",
                    "News and current events",
                    "Privacy-friendly queries"
                ],
                "serpapi_use_cases": [
                    "Structured data extraction",
                    "Rich metadata (featured snippets, knowledge graph)",
                    "Specialized searches (images, shopping, maps)",
                    "Deep research with search operators"
                ]
            }
        }
