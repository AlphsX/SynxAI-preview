...
from app.agent.langsmith_tracer import langsmith_tracer
...
class EnhancedChatService:
    def __init__(self):
        ...
        self.langsmith_tracer = langsmith_tracer  # LangSmith tracer for tracing
        ...
        # Conversation memory query patterns
        self.memory_query_patterns = [
            r'\b(what.*we.*talk.*about|what.*we.*discuss|what.*conversation|what.*topic)\b',
            r'\b(what.*i.*ask|what.*i.*said|what.*i.*mention|remind me)\b',
            r'\b(earlier|before|previous|last time|ago)\b',
            r'\b(continue|go back|return to|pick up where)\b',
            r'\b(summary|summarize|recap|review)\b'
        ]
    ...
    async def generate_ai_response(self, 
        ...
        """Generate AI response with enhanced context from external APIs and LangChain conversation memory"""
        ...
        try:
            ...
            if use_langchain_memory:
                ...
                # Convert LangChain history to our format
                ...
                logger.info(f"📚 Retrieved {len(conversation_history)} messages from LangChain memory")
                
                # Debug: Print conversation history
                if conversation_history:
                    logger.info(f"🔍 Conversation history preview:")
                    for i, msg in enumerate(conversation_history[-5:], 1):  # Last 5 messages
                        preview = msg["content"][:50] + "..." if len(msg["content"]) > 50 else msg["content"]
                        logger.info(f"  {i}. {msg['role']}: {preview}")
                else:
                    logger.info("⚠️ No conversation history found - this is the first message")
                
                # THEN: Add current user message to LangChain memory
                await self.memory_service.add_message_to_history(
                    session_id=session_id,
                    message=message,
                    is_user=True,
                    metadata={
                ...
                if self._is_memory_query(message):
                    logger.info("🔍 Detected memory query - providing conversation summary")
                    # Add conversation summary to enhanced context
                    enhanced_context["conversation_summary"] = await self._generate_conversation_summary(
            ...
            if self.groq_compound_service.should_use_compound_model(message):
                ...
                try:
                    ...
                    if extraction_result and extraction_result.get("success"):
                        ...
                        error_msg = extraction_result.get("error", "Website extraction failed") if extraction_result else "No extraction result"
            ...
            if self.use_langsmith_tracing and self.langsmith_tracer.is_enabled() and enhanced_context:
                await self.langsmith_tracer.trace_enhanced_context(
                    query=message,
            ...
            # 🧠 USE LANGCHAIN MEMORY FOR RESPONSE GENERATION
            if use_langchain_memory:
                logger.info("🤖 Generating response using LangChain Memory with streaming")
                
                # Build enhanced system prompt with context AND conversation awareness
                system_prompt = self._build_enhanced_system_message_with_memory(
                    enhanced_context, user_context, conversation_history
                )
                
                # Generate response using AI service with full conversation history
                ...
                try:
                    ...
                    logger.info(f"  - System message: {len(system_prompt)} chars")
                    logger.info(f"  - History messages: {len(messages) - 2}")
                    logger.info(f"  - Current user message: {message[:50]}...")
                    ...
                    trace_run_id = None
                    logger.info(f"🔍 LangSmith tracing check: use_tracing={self.use_langsmith_tracing}, tracer_enabled={self.langsmith_tracer.is_enabled()}")
                    if self.use_langsmith_tracing and self.langsmith_tracer.is_enabled():
                        logger.info("🔍 Starting LangSmith tracing for AI interaction")
                        try:
                            trace_run_id = await self.langsmith_tracer.trace_streaming_interaction(
                                messages=messages,
                                model_id=model_id,
                                metadata={
                                    "session_id": session_id,
                                    "user_id": user_context.get("user_id"),
                                    "enhanced_context": bool(enhanced_context),
                                    "context_types": list(enhanced_context.keys()) if enhanced_context else []
                                }
                            )
                            logger.info(f"✅ LangSmith trace started with ID: {trace_run_id}")
                        except Exception as trace_error:
                            logger.error(f"❌ Failed to start LangSmith trace: {trace_error}")
                    else:
                        logger.warning("⚠️ LangSmith tracing is disabled or tracer not available")
                    ...
                    # Complete LangSmith tracing if enabled
                    if trace_run_id and self.langsmith_tracer.is_enabled():
                        try:
                            await self.langsmith_tracer.update_streaming_trace(
                                run_id=trace_run_id,
                            ...
                            logger.info(f"✅ LangSmith trace completed: {trace_run_id}")
                        except Exception as trace_error:
                            logger.error(f"❌ Failed to complete LangSmith trace: {trace_error}")
                    
                    # Save both messages to LangChain memory after generation
                    await self.memory_service.add_message_to_history(
                        session_id=session_id,
                    ...
                    logger.info(f"✅ LangChain Memory: Response generated and saved to session {session_id}")
                    
                except Exception as langchain_error:
                    logger.error(f"LangChain memory error: {langchain_error}")
                    had_errors = True
                    error_details = {"langchain_error": str(langchain_error)}
                    
                    # Fallback to original method without memory
                    logger.info("🔄 Falling back to original AI service without memory")
                    ...
                    # Start LangSmith tracing if enabled (fallback case)
                    trace_run_id = None
                    if self.use_langsmith_tracing and self.langsmith_tracer.is_enabled():
                        logger.info("🔍 Starting LangSmith tracing for AI interaction (fallback)")
                        trace_run_id = await self.langsmith_tracer.trace_streaming_interaction(
                            messages=messages,
                            model_id=model_id,
                            metadata={
                ...
                # Original method for non-LangChain conversations
                ...
                try:
                    # Start LangSmith tracing if enabled (non-memory case)
                    trace_run_id = None
                    if self.use_langsmith_tracing and self.langsmith_tracer.is_enabled():
                        logger.info("🔍 Starting LangSmith tracing for AI interaction (non-memory)")
                        trace_run_id = await self.langsmith_tracer.trace_streaming_interaction(
                            messages=messages,
                            model_id=model_id,
                            metadata={
    ...
    def _is_memory_query(self, message: str) -> bool:
        """Determine if message is asking about conversation history"""
        message_lower = message.lower()
        return any(re.search(pattern, message_lower, re.IGNORECASE) for pattern in self.memory_query_patterns)
    
    async def _fetch_web_search_context(self, message: str) -> Dict[str, Any]:
        """Fetch web search context with intelligent routing"""
        try:
            # Use unified search service with intelligent routing
            search_results = await self.search_service.search_web(message, count=5)
            
            # Ensure search_results is not None
            if search_results is None:
                logger.warning(f"Search service returned None for query: {message}")
                search_results = []
            ...
            if search_results and isinstance(search_results, list) and len(search_results) > 0:
                ...
                provider = "unknown"
                if len(search_results) > 0 and search_results[0] and isinstance(search_results[0], dict):
                    provider = search_results[0].get("provider", "unknown")
                
                logger.info(f"Web search found {len(search_results)} results using {provider}")
                return {
                    "web_search": {
                        "query": message,
            ...
            logger.error(f"Web search context error: {e}", exc_info=True)
            return {}
    ...
    async def _fetch_news_context(self, message: str) -> Dict[str, Any]:
        """Fetch news context with intelligent routing"""
        try:
            # Use unified search service for news with intelligent routing
            news_results = await self.search_service.search_news(message, count=5)
            
            # Ensure news_results is not None
            if news_results is None:
                logger.warning(f"News search service returned None for query: {message}")
                news_results = []
            ...
            if news_results and isinstance(news_results, list) and len(news_results) > 0:
                # Safely get provider from first result
                provider = "unknown"
                if len(news_results) > 0 and news_results[0] and isinstance(news_results[0], dict):
                    provider = news_results[0].get("provider", "unknown")
                
                logger.info(f"News search found {len(news_results)} results using {provider}")
                return {
                    "news": {
                        "query": message,
            ...
            logger.error(f"News context error: {e}", exc_info=True)
            return {}
    ...
    async def _generate_conversation_summary(
        ...
        """Generate a summary of the conversation for memory queries"""
        try:
            if not conversation_history or len(conversation_history) == 0:
                return {
                    "summary": "This is the start of our conversation.",
                    "topics": [],
                    "message_count": 0
                }
            ...
            if len(user_messages) > 0:
                ...
                summary_parts.append(f"You started by asking: '{first_msg}'")
                
                if len(user_messages) > 1:
                    summary_parts.append(f"Most recently you asked: '{last_msg}'")
            ...
            topic_keywords = {
                "crypto": ["bitcoin", "ethereum", "crypto", "trading", "price"],
                "news": ["news", "latest", "update", "breaking"],
                "search": ["search", "find", "look up", "information"],
                "code": ["code", "programming", "function", "api"],
                "general": ["help", "question", "explain", "what", "how"]
            }
            ...
            return {
                "summary": " ".join(summary_parts) if summary_parts else "We've been having a conversation.",
    ...
    def _build_enhanced_system_message_with_memory(
        self, 
...
• Unclear separation between items"""
        ...
        # Conversation summary (for memory queries)
        if context.get("conversation_summary"):
            summary = context["conversation_summary"]
            context_sections.append("\n=== CONVERSATION SUMMARY ===")
            context_sections.append(f"Total messages: {summary.get('message_count', 0)}")
            if summary.get('topics'):
                context_sections.append(f"Topics discussed: {', '.join(summary.get('topics', []))}")
            context_sections.append(f"\n{summary.get('summary', 'We have been having a conversation.')}")
            context_sections.append("\nUse this information to answer questions about our conversation.")
            context_sections.append("")
    ...
    # ========================================
    # LangChain Memory Management Methods
    # ========================================
    
    def enable_langchain_memory(self):
        """Enable LangChain memory for all conversations"""
        self.use_langchain_memory = True
        logger.info("🧠 LangChain Memory enabled for all conversations")
    
    def disable_langchain_memory(self):
        """Disable LangChain memory and use original database method"""
        self.use_langchain_memory = False
        logger.info("💾 LangChain Memory disabled, using original database method")
    
    async def get_langchain_memory_stats(self, session_id: str) -> Dict[str, Any]:
        """Get LangChain memory statistics for a session"""
        try:
            return await self.memory_service.get_memory_stats(session_id)
        except Exception as e:
            logger.error(f"Error getting LangChain memory stats: {e}")
            return {"error": str(e)}
    
    async def clear_langchain_memory(self, session_id: str) -> bool:
        """Clear LangChain memory for a session"""
        try:
            await self.memory_service.clear_session_history(session_id)
            logger.info(f"🗑️ Cleared LangChain memory for session: {session_id}")
            return True
        except Exception as e:
            logger.error(f"Error clearing LangChain memory: {e}")
            return False
    
    async def get_langchain_conversation_history(self, session_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        """Get conversation history from LangChain memory"""
        try:
            return await self.memory_service.get_conversation_history(session_id, limit)
        except Exception as e:
            logger.error(f"Error getting LangChain conversation history: {e}")
            return []
    
    def is_langchain_memory_enabled(self) -> bool:
        """Check if LangChain memory is enabled"""
        return self.use_langchain_memory
