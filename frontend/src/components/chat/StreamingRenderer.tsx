  ...
  const processStreamingContent = useCallback(
    (newContent: string) => {
      ...
      try {
        ...
        const { content: processableContent, hasIncomplete } =
          bufferRef.current.getProcessableContent();
          ...
          <div className="p-4 bg-gradient-to-r from-red-50/80 to-pink-50/80 dark:from-red-900/20 dark:to-pink-900/20 border border-red-200/50 dark:border-red-700/50 rounded-lg backdrop-blur-sm">
            ...
            <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar">
              {errors.map((error, index) => (
                <div
                  key={index}
                  className="flex items-start space-x-2 p-2 bg-white/50 dark:bg-gray-800/50 rounded border border-red-200/30 dark:border-red-700/30"
                >
