...
export const detectMobileOS = (): MobileInfo => {
  ...
  // Tablet detection
  const isTablet =
    /iPad|Android.*Tablet|Windows.*Touch/i.test(userAgent) ||
...
export const getMobilePatterns = () => {
  return {
    ...
    enhanced:
      /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile|Tablet|MIUI|HarmonyOS|HyperOS|EMUI|Funtouch|ColorOS|OxygenOS|OneUI/i,
